...
[Script]
cron "5 7-13/2 * * *"  script-path=youth_readArticleAndVideo_7916.js,tag=中青看点阅读文章，视频-7916

*/

const $ = new Env("中青看点阅读文章，视频-7916")
...
if (youth.youthRead4_1 && youth.youthRead4_1.indexOf('&') > -1) {
    YouthArticleBodys = youth.youthRead4_1.split('&');
} else if (youth.youthRead4_1 && youth.youthRead4_1.indexOf('\n') > -1) {
    YouthArticleBodys = youth.youthRead4_1.split('\n');
} else {
    YouthArticleBodys = [youth.youthRead4_1]
}

if (youth.YOUTH_VIDEO4 && youth.YOUTH_VIDEO4.indexOf('&') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO4.split('&');
} else if (youth.YOUTH_VIDEO4 && youth.YOUTH_VIDEO4.indexOf('\n') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO4.split('\n');
} else {
    YouthVideoBodys = [youth.YOUTH_VIDEO4]
}

timebodyVal = youth.YOUTH_ATIME4


let dicItem = youth.YOUTH_GAIN_LOOK4
let lookArray = dicItem["youthLook"].split('\n');
let gainArray = dicItem["youthGain"].split('\n');
let currentAccount = dicItem["accountInfo"]

let dicItemAndroid = youth.YOUTH_GAIN_LOOK_ANDROID_4
