...
[Script]
cron "5 7-13/2 * * *"  script-path=youth_readArticleAndVideo_vx云淡风轻.js,tag=中青看点阅读文章，视频-vx云淡风轻

*/

const $ = new Env("中青看点阅读文章，视频-vx云淡风轻")
...
if (youth.youthRead12_1 && youth.youthRead12_1.indexOf('&') > -1) {
    YouthArticleBodys = youth.youthRead12_1.split('&');
} else if (youth.youthRead12_1 && youth.youthRead12_1.indexOf('\n') > -1) {
    YouthArticleBodys = youth.youthRead12_1.split('\n');
} else {
    YouthArticleBodys = [youth.youthRead12_1]
}

if (youth.YOUTH_VIDEO12 && youth.YOUTH_VIDEO12.indexOf('&') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO12.split('&');
} else if (youth.YOUTH_VIDEO12 && youth.YOUTH_VIDEO12.indexOf('\n') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO12.split('\n');
} else {
    YouthVideoBodys = [youth.YOUTH_VIDEO12]
}

timebodyVal = youth.YOUTH_ATIME12


let dicItem = youth.YOUTH_GAIN_LOOK12
let lookArray = dicItem["youthLook"].split('\n');
let gainArray = dicItem["youthGain"].split('\n');
let currentAccount = dicItem["accountInfo"]

let dicItemAndroid = youth.YOUTH_GAIN_LOOK_ANDROID_12
