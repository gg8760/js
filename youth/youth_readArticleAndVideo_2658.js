/*
[Script]
cron "5 7-13/2 * * *"  script-path=youth_readArticleAndVideo_2658.js,tag=中青看点阅读文章，视频-2658

*/

const $ = new Env("中青看点阅读文章，视频-2658")
...
if (youth.youthRead2_1 && youth.youthRead2_1.indexOf('&') > -1) {
    YouthArticleBodys = youth.youthRead2_1.split('&');
} else if (youth.youthRead2_1 && youth.youthRead2_1.indexOf('\n') > -1) {
    YouthArticleBodys = youth.youthRead2_1.split('\n');
} else {
    YouthArticleBodys = [youth.youthRead2_1]
}

if (youth.YOUTH_VIDEO2 && youth.YOUTH_VIDEO2.indexOf('&') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO2.split('&');
} else if (youth.YOUTH_VIDEO2 && youth.YOUTH_VIDEO2.indexOf('\n') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO2.split('\n');
} else {
    YouthVideoBodys = [youth.YOUTH_VIDEO2]
}

timebodyVal = youth.YOUTH_ATIME2

let dicItem = youth.YOUTH_GAIN_LOOK2

let lookArray = dicItem["youthLook"].split('\n');
let gainArray = dicItem["youthGain"].split('\n');
let currentAccount = dicItem["accountInfo"]

let dicItemAndroid = youth.YOUTH_GAIN_LOOK_ANDROID_2
                ...
                try {
                    ...
                    if (readres.items.complete == 1) {
                        ...
                        if(readres.items.max_notice.indexOf("看太久了") > -1) {
                            ...
                            console.log('breakCountArticle=',breakCountArticle)
