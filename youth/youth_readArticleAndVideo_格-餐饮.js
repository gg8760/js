...
[Script]
cron "5 7-13/2 * * *"  script-path=youth_readArticleAndVideo_格-餐饮.js,tag=中青看点阅读文章，视频-格-餐饮

*/

const $ = new Env("中青看点阅读文章，视频-格-餐饮")
...
if (youth.youthRead14_1 && youth.youthRead14_1.indexOf('&') > -1) {
    YouthArticleBodys = youth.youthRead14_1.split('&');
} else if (youth.youthRead14_1 && youth.youthRead14_1.indexOf('\n') > -1) {
    YouthArticleBodys = youth.youthRead14_1.split('\n');
} else {
    YouthArticleBodys = [youth.youthRead14_1]
}

if (youth.YOUTH_VIDEO14 && youth.YOUTH_VIDEO14.indexOf('&') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO14.split('&');
} else if (youth.YOUTH_VIDEO14 && youth.YOUTH_VIDEO14.indexOf('\n') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO14.split('\n');
} else {
    YouthVideoBodys = [youth.YOUTH_VIDEO14]
}

timebodyVal = youth.YOUTH_ATIME14


let dicItem = youth.YOUTH_GAIN_LOOK14
let lookArray = dicItem["youthLook"].split('\n');
let gainArray = dicItem["youthGain"].split('\n');
let currentAccount = dicItem["accountInfo"]

let dicItemAndroid = youth.YOUTH_GAIN_LOOK_ANDROID_14
