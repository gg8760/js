...
const $ = new Env("中青看点阅读文章，视频8-格vx0306")
...
if (youth.youthRead8_1 && youth.youthRead8_1.indexOf('&') > -1) {
    YouthArticleBodys = youth.youthRead8_1.split('&');
} else if (youth.youthRead8_1 && youth.youthRead8_1.indexOf('\n') > -1) {
    YouthArticleBodys = youth.youthRead8_1.split('\n');
} else {
    YouthArticleBodys = [youth.youthRead8_1]
}

if (youth.YOUTH_VIDEO8 && youth.YOUTH_VIDEO8.indexOf('&') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO8.split('&');
} else if (youth.YOUTH_VIDEO8 && youth.YOUTH_VIDEO8.indexOf('\n') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO8.split('\n');
} else {
    YouthVideoBodys = [youth.YOUTH_VIDEO8]
}

timebodyVal = youth.YOUTH_ATIME8

let dicItem = youth.YOUTH_GAIN_LOOK8
let lookArray = dicItem["youthLook"].split('\n');
let gainArray = dicItem["youthGain"].split('\n');
let currentAccount = dicItem["accountInfo"]

let dicItemAndroid = youth.YOUTH_GAIN_LOOK_ANDROID_8
