...
[Script]
cron "5 7-13/2 * * *"  script-path=youth_readArticleAndVideo_7585.js,tag=中青看点阅读文章，视频-7585

*/

const $ = new Env("中青看点阅读文章，视频-7585")
...
if (youth.youthRead6_1 && youth.youthRead6_1.indexOf('&') > -1) {
    YouthArticleBodys = youth.youthRead6_1.split('&');
} else if (youth.youthRead6_1 && youth.youthRead6_1.indexOf('\n') > -1) {
    YouthArticleBodys = youth.youthRead6_1.split('\n');
} else {
    YouthArticleBodys = [youth.youthRead6_1]
}

if (youth.YOUTH_VIDEO6 && youth.YOUTH_VIDEO6.indexOf('&') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO6.split('&');
} else if (youth.YOUTH_VIDEO6 && youth.YOUTH_VIDEO6.indexOf('\n') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO6.split('\n');
} else {
    YouthVideoBodys = [youth.YOUTH_VIDEO6]
}

timebodyVal = youth.YOUTH_ATIME6

let dicItem = youth.YOUTH_GAIN_LOOK6
let lookArray = dicItem["youthLook"].split('\n');
let gainArray = dicItem["youthGain"].split('\n');
let currentAccount = dicItem["accountInfo"]

let dicItemAndroid = youth.YOUTH_GAIN_LOOK_ANDROID_6
