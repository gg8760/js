...
[Script]
cron "5 7-13/2 * * *"  script-path=youth_readArticleAndVideo_格vx0306.js,tag=中青看点阅读文章，视频-格vx0306
...
if (youth.youthRead7_1 && youth.youthRead7_1.indexOf('&') > -1) {
    YouthArticleBodys = youth.youthRead7_1.split('&');
} else if (youth.youthRead7_1 && youth.youthRead7_1.indexOf('\n') > -1) {
    YouthArticleBodys = youth.youthRead7_1.split('\n');
} else {
    YouthArticleBodys = [youth.youthRead7_1]
}

if (youth.YOUTH_VIDEO7 && youth.YOUTH_VIDEO7.indexOf('&') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO7.split('&');
} else if (youth.YOUTH_VIDEO7 && youth.YOUTH_VIDEO7.indexOf('\n') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO7.split('\n');
} else {
    YouthVideoBodys = [youth.YOUTH_VIDEO7]
}

timebodyVal = youth.YOUTH_ATIME7


let dicItem = youth.YOUTH_GAIN_LOOK7
let lookArray = dicItem["youthLook"].split('\n');
let gainArray = dicItem["youthGain"].split('\n');
let currentAccount = dicItem["accountInfo"]

let dicItemAndroid = youth.YOUTH_GAIN_LOOK_ANDROID_7
