...
[Script]
cron "5 7-13/2 * * *"  script-path=youth_readArticleAndVideo_vx慕言.js,tag=中青看点阅读文章，视频-vx慕言

*/

const $ = new Env("中青看点阅读文章，视频-vx慕言")
...
if (youth.youthRead9_1 && youth.youthRead9_1.indexOf('&') > -1) {
    YouthArticleBodys = youth.youthRead9_1.split('&');
} else if (youth.youthRead9_1 && youth.youthRead9_1.indexOf('\n') > -1) {
    YouthArticleBodys = youth.youthRead9_1.split('\n');
} else {
    YouthArticleBodys = [youth.youthRead9_1]
}

if (youth.YOUTH_VIDEO9 && youth.YOUTH_VIDEO9.indexOf('&') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO9.split('&');
} else if (youth.YOUTH_VIDEO9 && youth.YOUTH_VIDEO9.indexOf('\n') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO9.split('\n');
} else {
    YouthVideoBodys = [youth.YOUTH_VIDEO9]
}

timebodyVal = youth.YOUTH_ATIME9

let dicItem = youth.YOUTH_GAIN_LOOK9
let lookArray = dicItem["youthLook"].split('\n');
let gainArray = dicItem["youthGain"].split('\n');
let currentAccount = dicItem["accountInfo"]

let dicItemAndroid = youth.YOUTH_GAIN_LOOK_ANDROID_9
