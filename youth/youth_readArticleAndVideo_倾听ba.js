...
[Script]
cron "5 7-13/2 * * *"  script-path=youth_readArticleAndVideo_倾听ba.js,tag=中青看点阅读文章，视频-11-倾听_ba

*/

const $ = new Env("中青看点阅读文章，视频-11-倾听_ba")
...
if (youth.youthRead11_1 && youth.youthRead11_1.indexOf('&') > -1) {
    YouthArticleBodys = youth.youthRead11_1.split('&');
} else if (youth.youthRead11_1 && youth.youthRead11_1.indexOf('\n') > -1) {
    YouthArticleBodys = youth.youthRead11_1.split('\n');
} else {
    YouthArticleBodys = [youth.youthRead11_1]
}

if (youth.YOUTH_VIDEO11 && youth.YOUTH_VIDEO11.indexOf('&') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO11.split('&');
} else if (youth.YOUTH_VIDEO11 && youth.YOUTH_VIDEO11.indexOf('\n') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO11.split('\n');
} else {
    YouthVideoBodys = [youth.YOUTH_VIDEO11]
}

timebodyVal = youth.YOUTH_ATIME11

let dicItem = youth.YOUTH_GAIN_LOOK11
let lookArray = dicItem["youthLook"].split('\n');
let gainArray = dicItem["youthGain"].split('\n');
let currentAccount = dicItem["accountInfo"]

let dicItemAndroid = youth.YOUTH_GAIN_LOOK_ANDROID_11
