...
!(async () => {
    ...
    else
    {
        ...
        zqkdCookieArr = youth.YOUTH_HEADER.split('\n');

        for(let j=0; j<zqkdCookieArr.length; j++) {
            
            userCk = zqkdCookieArr[j].trim()
            ...
            if(newsItem) {
                ...
                for(let i=0; i<zqkdShareNum; i++) {
                    ...
                    let randomTime = getRandomInt(20,40)
                    ...
                    si = randomString(32)
                    console.log(`--随机延迟${randomTime}秒后开始模拟第${readCount}次分享阅读`)
...
async function listsNewTag() {
    ...
    if(result.status == 1) {
        if(result.data && result.data.items && Array.isArray(result.data.items)) {
            ...
            // console.log('shareIdx----',shareIdx);
            // console.log('newsItem___',newsItem);
            await $.wait(1000)
...
// Storage -> visit-> callback -> open
// https://script.baertt.com/count2/storage?t=&referer=https%253A%252F%252Ffocu.youth.cn%252Fwaphotfour%252F20220212%253Fsid%253D41981554%2526uid%253D62636814%2526timestamp%253D1644904854%2526signature%253DVMYqRmZGljQD8pAPB3aD9YDlMtD3m3Q1Xy9O06evWJwbxoN5KL%2526share_id%253D62636814419815541644904858005%2526scene_id%253Dfire_share%2526time%253D1644904858005%2526uid%253D62636814%2526phone_code%253De37d45e159ebeec7af974d67add3e2a1%2526time%253D1644904858%2526app_version%253D2.3.1%2526sign%253D5381f8fffbaa079523232f83bd7ecf15&_=1644906809268&jsonpcallback=jsonp2
...
async function shareReadStep2() {
    ...
    let rndtime = Math.floor(new Date().getTime())
    let share_url = encodeURIComponent(encodeURIComponent(newsItem.share_url+'#'))
    let shareLink = `https://script.baertt.com/count2/visit?type=1&si=${si}&referer=${share_url}&_=${rndtime}&jsonpcallback=jsonp5`
    let urlObject = populateGetUrlRead(shareLink)
    await httpGet(urlObject,caller)
}
...
async function shareReadStep3() {
    ...
    let rndtime = Math.floor(new Date().getTime())
    let share_url = encodeURIComponent(encodeURIComponent(newsItem.share_url+'#'))
    let shareLink = `https://script.baertt.com/count2/openpage?referer=${share_url}&_=${rndtime}&jsonpcallback=jsonp7`
    let urlObject = populateGetUrlRead(shareLink)
    await httpGet(urlObject,caller)
}
...
async function shareReadStep4() {
    ...
    let rndtime = Math.floor(new Date().getTime())
    let share_url = encodeURIComponent(encodeURIComponent(newsItem.share_url+'#'))
    let shareLink = `https://script.baertt.com/count2/callback?si=${si}&referer=${share_url}&_=${rndtime}&jsonpcallback=jsonp4`
    let urlObject = populateGetUrlRead(shareLink)
    await httpGet(urlObject,caller)
}
