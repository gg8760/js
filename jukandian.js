...
if (typeof $request !== 'undefined') {
  ...
  !(async () => {
    ...
    // 1 .峰   2. 格格 3. 7585  4. 4564

    let jukan = require('./raw_jukan_account_parameter');

    let accountArray = jukan.jukan_account_info
    console.log(`您共提供${accountArray.length}个聚看点账号\n`)


    for (let i = 0; i < accountArray.length; i++) {
      if (accountArray[i]) {
        let dicItem = accountArray[i]
        ID = dicItem["openId"]

        let accountNum = dicItem["accountInfo"]
        ...
        let path = {
          ...
          "appversioncode": "565",
          "time": String(Math.floor((new Date()).valueOf() / 1000)),
          "upopenid": "",
          "apptoken": "xzwltoken070704",
          "openid": ID,
          "os": "iOS",
          "uniqueid": "C132096C-6D9C-4234-BC75-95A85535EABB",
          "appversion": "5.6.5",
          "umengid": "5a330656e691021f44baddc7ed3c84",
          "memid": "",
          "smid": "D2jy3JzSUpMqElfVdhMLL2nyJAyOScEYghakzOYA5sy+wX8c"
        }
        ...
        apptoken = path.apptoken
        appVersion = path.appversion
        cookieval = `xz_jkd_appkey=${ID}!iOS!${appVersion}`

        bodyval = "jsondata=" + encodeURIComponent(JSON.stringify(path))
        bodyvalVideo = "jsondata=" + encodeURIComponent(JSON.stringify(videopath))
        ...
        // console.log("curcash",curcash)
        // console.log("drawcash",drawcash)
  ...
  return new Promise((resolve, reject) => {
    ...
    $.post(infourl, async (error, resp, data) => {
      try {
        ...
        // console.log("artTotalartTotalartTotal----")
        // console.log(data)
        // console.log("artTotalartTotalartTotal----")
