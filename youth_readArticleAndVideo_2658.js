...
!(async () => {
    ...
    $.log(`\n~~~~~~~~~~~~~账号2658共提供${YouthArticleBodys.length}次阅读文章任务~~~~~~~~~~~~~`)
    $.log(`~~~~~~~~~~~~~账号2658共提供${YouthVideoBodys.length}次阅读视频任务~~~~~~~~~~~~~\n`)
        ...
        for (let i = 0; i < YouthArticleBodys.length; i++) {
            if (breakReadArticle == false) {
                if (YouthArticleBodys[i]) {
                    articlebody = YouthArticleBodys[i];
                    $.index = i + 1;
                    $.log(`-------------------------\n开始中青看点第${$.index}次阅读\n`);
                    await bodyInfo();
                }
...
function readTime(timebodyVal) {
    ...
    return new Promise((resolve, reject) => {
        $.post(gainHost('user/stay.json', timebodyVal), (error, resp, data) => {
