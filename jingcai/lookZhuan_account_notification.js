...
!(async () => {
    ...
    let youth = require('./lookZhuan_parameter');

    let accountInfoArray = youth.LOOK_ACOUNT_INFO_URL
    let cookieArray = youth.LOOK_HEADER.split('\n');

    for (let index = 0; index < accountInfoArray.length; index++) {
        let dicItem = accountInfoArray[index];
        cookie = cookieArray[index].trim();
        ...
        if (accountInfoUrl.length > 0) {
            await accountInfo(accountInfoUrl);
        }

        if (withdrawInfoUrl.length > 0) {
    ...
    notifyMessage = notifyMessage + `\n\n【已到账总金额💰】${totalArrivedMoney.toFixed(2)} 元\n` + `【审核中总金额💰】${totalReviewMoney}元\n\n`
    console.log("通知推送信息-----------\n\n",notifyMessage)
...
.finally(() => $.done())
    ...
    function withdrawInfo(withdrawUrl) {
        return new Promise((resolve, reject) => {
            $.get(batHost(withdrawUrl + cookie), async (error, resp, data) => {
