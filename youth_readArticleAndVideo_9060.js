...
const $ = new Env("中青看点阅读文章，视频-9060")
...
if (youth.youthRead1_1 && youth.youthRead1_1.indexOf('&') > -1) {
    YouthArticleBodys = youth.youthRead1_1.split('&');
} else if (youth.youthRead1_1 && youth.youthRead1_1.indexOf('\n') > -1) {
    YouthArticleBodys = youth.youthRead1_1.split('\n');
} else {
    YouthArticleBodys = [youth.youthRead1_1]
}

if (youth.YOUTH_VIDEO && youth.YOUTH_VIDEO.indexOf('&') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO.split('&');
} else if (youth.YOUTH_VIDEO && youth.YOUTH_VIDEO.indexOf('\n') > -1) {
    YouthVideoBodys = youth.YOUTH_VIDEO.split('\n');
} else {
    YouthVideoBodys = [youth.YOUTH_VIDEO]
}

timebodyVal = youth.YOUTH_ATIME
...
!(async () => {
    ...
    $.log(`\n~~~~~~~~~~~~~账号1-9060共提供${YouthArticleBodys.length}次阅读文章任务~~~~~~~~~~~~~`)
    $.log(`~~~~~~~~~~~~~账号1-9060共提供${YouthVideoBodys.length}次阅读视频任务~~~~~~~~~~~~~\n`)
