    ...
    return new Promise((resolve, reject) => {
      $.get(apiHost(`v1/user/info?token=${cookie}`), async (error, resp, data) => {
        ...
        //   console.log(obj)
          ...
          let account = `【${accountInfo}】${obj.data.name}\n`
          let code = `【邀请码】${obj.data.invite_code}\n`
          let mobile = `【手机】${obj.data.mobile}\n`
          let money = `【当前价值💰】${obj.data.money}\n`
          let withdraw = `【提现账号】${accountWithdraw}\n`

          notifyMessage = notifyMessage + account + code + mobile + money + withdraw
...
// 获取提现信息
  // https://api.st615.com/v1/cash/page-list?token=kPyT9AWbetDXgseHslLjU4S2F-sya505&page=1&limit=10
    ...
    return new Promise((resolve, reject) => {
      $.get(apiHost(`v1/cash/page-list?token=${cookie}&page=1&limit=100`), async (error, resp, data) => {
        try {
          ...
          let withdrawMoney = 0.0;
          for (let index = 0; index < array.length; index++) {
            const element = array[index];
            if (element.status == 1 && element.content?.indexOf('SUCCESS') > -1) {
              withdrawMoney = withdrawMoney + element.money
            }
          }
          
          let withdraw = `【已到账money💰】${withdrawMoney.toFixed(2)}元\n\n `
          notifyMessage = notifyMessage + withdraw
