...
[Script]
cron "0 8 * * *"  script-path=jiuzhang_account_withdraw.js,tag=九章头条-提现
...
function articleList_android(type) {
  ...
  return new Promise((resolve, reject) => {
    ...
    $.get(apiHost(`v2/article/list?token=${cookie}&page=1&limit=20&title=&cid=0&type=${type}&terminal=vivo&version=1.8.12`), async (error, resp, data) => {
      try {
        let obj = JSON.parse(data)
        let array = obj.data.list
        // console.log(array)
...
// https://api.st615.com/v2/cash/qualify?token=38RH-bvbFxsLHRq7GefCeP0-87i2yNK5&money=0.3
function withdrawQualify() {
  console.log(`\n🍒 验证账号${accountInfo} 是否满足提现要求🍒\n`)
  return new Promise((resolve, reject) => {
    $.get(apiHost(`v2/cash/qualify?money=${withdrawMoney}&token=${cookie}`), async (error, resp, data) => {
      try {
        let obj = JSON.parse(data)
        let array = obj.data.list
        console.log(array)
...
// 分享
// URL	https://api.st615.com/v2/share/info?token=iXTqHlC2zxr_JVRMXqq3CnvAIk3DsraT&type=1
// https://api.st615.com/v2/share/info?token=${cookie}&type=1
...
// https://api.st615.com/v2/article/share

function shareWechat2() {
  console.log(`\n🍒 账号${accountInfo} 分享微信 2🍒\n`)
  return new Promise((resolve, reject) => {
    $.post(apiHost(`v2/article/share`, `device=iPhone&token=${cookie}&source=cash&os=14.0.1&id=`), async (error, resp, data) => {
      try {
        let obj = JSON.parse(data)
        console.log(obj)
        await $.wait(5 * 1000);
...
function shareWechat2_android() {
  console.log(`\n🍒 账号${accountInfo} 分享微信 2🍒\n`)
  return new Promise((resolve, reject) => {
    $.post(apiHost(`v2/article/share`, `device=vivo Y55A&token=${cookie}&source=cash&os=Android 6.0.1&id=`), async (error, resp, data) => {
      try {
        let obj = JSON.parse(data)
        console.log(obj)
        await $.wait(5 * 1000);
...
// 提现
// https://api.st615.com/v2/cash/withdraw-new
// token	55nLV5tA48s-Qjr5uvlSw3o9p1W-BW0_
// type	1
// money	0.3
...
function withdraw() {
  console.log("\n🍒 去提现 🍒\n")
  return new Promise((resolve, reject) => {
    $.post(apiHost(`v2/cash/withdraw-new`, `type=1&token=${cookie}&money=${withdrawMoney}`), async (error, resp, data) => {
      try {
        let obj = JSON.parse(data)
        console.log(obj)
        await $.wait(5 * 1000);
