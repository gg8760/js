...
const $iosrule = new Env('惠头条资产通知');//声明必须
...
!(async () => {
  ...
  for (let index = 0; index < accountArray.length; index++) {
    ...
    let dicItem = accountArray[index]
  ...
  console.log("通知推送信息-----------\n\n", notifyMessage)
    ...
    //   await notify.sendNotify(`聚看点资产通知`, notifyMessage);
    // }
...
function accountInfo() {
  ...
  //     "todayIncome": 14031,
  //     "total": 359789,
  //     "withDrawRemaining": 259789
  //   }
  // }
  return new Promise((resolve, reject) => {
    const profiturl = {
      url: "https://api.cashtoutiao.com/frontend/credit/summary?" + pathUrl,
      headers: {
    ...
    $iosrule.post(profiturl, async (error, resp, data) => {
      try {
        ...
        let accountName = `【${accountCurrent}】${accountMobile}\n`
        // let withdraw = `【提现账号】${withdrawAccount}\n`
        let score = `【今日金币】${obj.userCreditSummary.todayIncome}\n`
        let currentCash = `【我的金币】${obj.userCreditSummary.withDrawRemaining}\n`
        
        let withdrawScore = (parseFloat(obj.userCreditSummary.total) - parseFloat(obj.userCreditSummary.withDrawRemaining)).toFixed(2)
        let withdrawMoney = `【已提现金额】${withdrawScore / 10000}\n\n`

        notifyMessage = notifyMessage + accountName + score + currentCash + withdrawMoney
